...
export function Newsletter() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    if (IS_STATIC_EXPORT) {
      ...
      setErrorMessage('Newsletter subscription coming soon!');
      ...
      <form onSubmit={handleSubscribe} className='flex flex-col gap-2 max-w-64'>
        <div className='relative'>
          <input
            ...
            required
            className='w-full px-3 py-2 bg-gray-200 text-black text-sm border rounded-md focus:outline-none focus:ring-2 focus:ring-primary disabled:opacity-50'
            disabled={subscribeStatus === 'loading' || IS_STATIC_EXPORT}
          />
        </div>
        <Button
          type='submit'
        ...
        >
          {subscribeStatus === 'loading' ? (
            ...
            <>
              <Rocket className='w-3.5 h-3.5 mr-1' /> Coming Soon
            </>
          ) : (
            <>
