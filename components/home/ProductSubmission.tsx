...
export default function ProductSubmission() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (IS_STATIC_EXPORT) {
      setStatus('error');
      setErrorMessage('Submission feature coming soon!');
              ...
              <div className='space-y-4'>
                <div>
                  ...
                  <input
                    ...
                    required
                    className='w-full px-4 py-3 bg-white/20 text-white placeholder-white/50 border border-white/30 rounded-xl focus:outline-none focus:ring-2 focus:ring-white/50 transition-all disabled:opacity-50'
                    disabled={status === 'loading' || IS_STATIC_EXPORT}
                  />
                </div>
                <Button
                  type='submit'
                  disabled={status === 'loading' || IS_STATIC_EXPORT}
                  className='w-full h-12 bg-white hover:bg-white/90 text-indigo-600 font-semibold rounded-xl transition-all duration-200 hover:shadow-lg disabled:opacity-50'
                >
                  {status === 'loading' ? (
